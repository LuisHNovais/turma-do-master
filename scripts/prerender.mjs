...
import { BRAND_NAME, EXPORT_PREFIX } from '../src/lib/brand.js';
import {
...
const fileName = (...parts) => `${[EXPORT_PREFIX, ...parts.filter(Boolean)].join('-')}`;
const conversationFile = (entry, ext) => `${fileName(entry.id)}.${ext}`;
const monthFile = (entry, month, ext) => `${fileName(entry.id, month)}.${ext}`;
const allFile = (ext) => `${EXPORT_PREFIX}.${ext}`;
const zipFile = `${EXPORT_PREFIX}-export.zip`;
...
function describe(entry, profile, who) {
  ...
  const fallback = `Conversa entre Daniel Vorcaro e ${who}: ${entry.total_messages} mensagens, de ${entry.date_range.start} a ${entry.date_range.end}, no ${BRAND_NAME}.`;
...
function conversationLd(entry, who, description, { path, month, first, last, parent } = {}) {
  ...
  const data = {
    ...
    encoding: [
      { '@type': 'MediaObject', encodingFormat: 'text/markdown', contentUrl: `${SITE}/export/${conversationFile(entry, 'md')}` },
      { '@type': 'MediaObject', encodingFormat: 'application/json', contentUrl: `${SITE}/export/${conversationFile(entry, 'json')}` },
    ],
    dateModified: today,
  };
...
function conversationPage(entry, messages, profile, who) {
  ...
  out.push(`<p>${entry.total_messages} mensagens, de ${entry.date_range.start} a ${entry.date_range.end}. `
    + `<a href="/#/chat/${entry.id}">Abrir no ${BRAND_NAME}</a> · `
    + `<a href="/export/${conversationFile(entry, 'md')}">Markdown completo</a> · `
    + `<a href="/export/${conversationFile(entry, 'json')}">JSON</a></p>`);
  ...
  if (paged) {
    out.push(`<p>As outras ${messages.length - shown.length} mensagens estão nas páginas por mês acima, no <a href="/export/${conversationFile(entry, 'md')}">Markdown completo</a> e <a href="/#/chat/${entry.id}">no app</a>.</p>`);
  }
  ...
  return appPage({
    title: `Daniel Vorcaro ↔ ${who} — ${BRAND_NAME}`,
    description,
    path,
...
function monthPage(entry, who, ym, msgs, prevYm, nextYm) {
  ...
  out.push(`<p>${msgs.length} mensagens em ${longMonth(ym)}, de ${msgs[0].date} a ${msgs.at(-1).date}. `
    ...
    + `<a href="/#/chat/${entry.id}/msg/${msgs[0].id}">Abrir no ${BRAND_NAME}</a> · `
    + `<a href="/export/${monthFile(entry, ym, 'md')}">Markdown deste mês</a> · `
    + `<a href="/export/${conversationFile(entry, 'md')}">Markdown completo</a></p>`);
  ...
  return appPage({
    title: `Daniel Vorcaro ↔ ${who} — ${longMonth(ym)} — ${BRAND_NAME}`,
    description,
    path,
...
<link rel="icon" type="image/svg+xml" href="/assets/turma-master-logo.svg">
...
<body>
<main>
<header><a href="/">${BRAND_NAME}</a> · <a href="/quem">Pessoas citadas</a></header>
${body}
</main>
</body>
</html>
...
function personPage(person, mentions) {
  ...
  return standalone({ title: `${person.name} nas conversas de Daniel Vorcaro — ${BRAND_NAME}`, description, path, jsonLd, body: body.join('\n') });
}
...
function peopleIndex(people) {
  ...
  return standalone({ title: `Pessoas citadas — ${BRAND_NAME}`, description: 'Quem aparece nas conversas de Daniel Vorcaro, com toda menção datada e apontando para a mensagem.', path: '/quem', jsonLd, body: body.join('\n') });
}
...
function llmsFull(built, people) {
  ...
  for (const { entry, who, profile, months } of built) {
    ...
    if (months) out.push(`- Um mês por página: ${[...months.keys()].map(ym => `${SITE}/chat/${entry.id}/${ym}`).join(' · ')}`, `- Markdown por mês (troque o mês): ${SITE}/export/${monthFile(entry, [...months.keys()][0], 'md')}`);
    out.push(`- Conversa completa em Markdown: ${SITE}/export/${conversationFile(entry, 'md')} · JSON: ${SITE}/export/${conversationFile(entry, 'json')}`);
  ...
  out.push('## Export', '',
    ...
    `- Zip com tudo: ${SITE}/export/${zipFile} (3 MB)`,
    `- Tudo em Markdown: ${SITE}/export/${allFile('md')} (3 MB)`,
    `- Tudo em JSON: ${SITE}/export/${allFile('json')} (14 MB)`, '');
  return out.join('\n');
}
...
function sitemap(built, people) {
  ...
  rows.push(url(`${SITE}/export/${allFile('md')}`, '0.6', 'monthly'));
  for (const { entry } of built) rows.push(url(`${SITE}/export/${conversationFile(entry, 'md')}`, '0.5', 'monthly'));
...
const SIZES = { [allFile('md')]: mb(allFile('md')), [allFile('json')]: mb(allFile('json')), [zipFile]: mb(zipFile) };
const stampSizes = (text) => text.replace(new RegExp(`(${EXPORT_PREFIX}(?:-export)?\\.(?:md|json|zip)) \\([\\d.,]+ MB\\)`, 'g'), (m, name) => `${name} (${SIZES[name] || m.slice(name.length + 2, -1)})`);
