 ...
 *
 * Runs after split-data.mjs, because the list of conversations and their ids
 ...
 *
 * The contact profiles live in src/lib/profile-content.js. Importing them keeps
 * the export and the site saying the same thing — a profile edited in one place
 * is edited in both.
 */
...
function conversationMarkdown(entry, messages, { standalone = true, month = null } = {}) {
  ...
  if (standalone) {
    out.push(`> Exportado de [${BRAND_NAME}](${SITE}/#/chat/${entry.id}) em ${generatedAt.slice(0, 10)}.${month ? ` Só ${monthName(month)} (${messages.length} mensagens); a conversa inteira está em ${conversationFile(entry, 'md')}.` : ''} Código e dados: ${REPO}.`, '');
  }
...
function readme(conversations) {
  ...
  return [
    `# ${BRAND_NAME} — export completo`, '',
    `Gerado em ${generatedAt.slice(0, 10)} a partir de ${SITE}. ${conversations.length} conversas, ${total} mensagens.`, '',
    '## O que tem aqui', '',
    '| Arquivo | Conteúdo |', '|---|---|',
    `| \`${EXPORT_PREFIX}-<conversa>.md\` | Uma conversa legível: proveniência, quem é o contato (com fontes) e as mensagens, dia a dia |`,
    `| \`${EXPORT_PREFIX}-<conversa>.json\` | A mesma conversa como dados: metadados, perfil e todas as mensagens com os campos originais |`,
    `| \`${allFile('md')}\` / \`${allFile('json')}\` | Tudo num arquivo só |`, '',
    '## Como ler', '',
...
function allMarkdown(built) {
  ...
  const out = [
    `# ${BRAND_NAME} — todas as conversas`, '',
    `> Exportado de [${BRAND_NAME}](${SITE}) em ${generatedAt.slice(0, 10)}. ${built.length} conversas, ${total} mensagens. Código e dados: ${REPO}.`, '',
    aboutMarkdown(),
...
for (const stale of readdirSync(OUT)) {
  if (stale.startsWith(EXPORT_PREFIX) || stale.startsWith('masterwhats')) rmSync(join(OUT, stale), { recursive: true, force: true });
}
...
for (const entry of entries) {
  ...
  writeFileSync(join(OUT, conversationFile(entry, 'md')), mdText);
  writeFileSync(join(OUT, conversationFile(entry, 'json')), jsonText);
  zip.file(conversationFile(entry, 'md'), mdText);
  zip.file(conversationFile(entry, 'json'), jsonText);
  ...
  if (isPaged(entry)) {
    ...
    for (const [ym, msgs] of months) {
      ...
      writeFileSync(join(OUT, monthFile(entry, ym, 'md')), monthMd);
      writeFileSync(join(OUT, monthFile(entry, ym, 'json')), monthJson);
      zip.file(`meses/${monthFile(entry, ym, 'md')}`, monthMd);
    }
...
writeFileSync(join(OUT, allFile('md')), allMd);
writeFileSync(join(OUT, allFile('json')), allJson);
...
writeFileSync(join(OUT, zipFile), zipBuf);

console.log(`\nDone! ${built.length} conversations → ${OUT}`);
console.log(`${allFile('md')} ${(allMd.length / 1048576).toFixed(1)} MB, ${allFile('json')} ${(allJson.length / 1048576).toFixed(1)} MB, zip ${(zipBuf.length / 1048576).toFixed(1)} MB`);
