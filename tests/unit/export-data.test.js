...
import { EXPORT_PREFIX } from '../../src/lib/brand.js';
...
const md = (id) => readFileSync(join(EXPORT_DIR, `${EXPORT_PREFIX}-${id}.md`), 'utf-8');
const json = (id) => JSON.parse(readFileSync(join(EXPORT_DIR, `${EXPORT_PREFIX}-${id}.json`), 'utf-8'));

describe('one pair of files per conversation', () => {
  it('writes a .md and a .json for every conversation the site lists', () => {
    for (const conv of conversations) {
      expect(existsSync(join(EXPORT_DIR, `${EXPORT_PREFIX}-${conv.id}.md`)), conv.id).toBe(true);
      expect(existsSync(join(EXPORT_DIR, `${EXPORT_PREFIX}-${conv.id}.json`)), conv.id).toBe(true);
    }
  });
});
...
describe('the JSON', () => {
  ...
  it('says where it came from', () => {
    for (const conv of conversations) {
      ...
      expect(meta.site).toBe('https://turmadomaster.vercel.app');
...
describe('the Markdown', () => {
  ...
  it('turns every highlight into a link with date and page', () => {
    const text = md('alexandre-de-moraes');
    expect(text).toContain('](https://turmadomaster.vercel.app/chat/alexandre-de-moraes#msg-');
    expect(text).toMatch(/⟨\d{2}\/\d{2}\/\d{4} \d{2}:\d{2} · laudo p\. \d+, fig\. \d+⟩/);
    expect(text).toContain('218 páginas');
    expect(text).toContain('[no repositório](https://github.com/LuisHNovais/turma-do-master/blob/main/data/source/');
  });
...
describe('the big conversation, one month at a time', () => {
  it('writes a Markdown and a JSON per month, each saying what it is', () => {
    const md = readFileSync(join(EXPORT_DIR, `${EXPORT_PREFIX}-martha-graeff-2024-12.md`), 'utf-8');
    ...
    const j = JSON.parse(readFileSync(join(EXPORT_DIR, `${EXPORT_PREFIX}-martha-graeff-2024-12.json`), 'utf-8'));
    expect(j.month).toBe('2024-12');
    expect(j.messages.length).toBe(lines.length);
  });

  it('does not split the small ones', () => {
    expect(existsSync(join(EXPORT_DIR, `${EXPORT_PREFIX}-ciro-soares-2025-03.md`))).toBe(false);
  });
});

describe('everything at once', () => {
  it('writes one Markdown with every conversation in it', () => {
    const text = readFileSync(join(EXPORT_DIR, `${EXPORT_PREFIX}.md`), 'utf-8');
    for (const conv of conversations) {
  ...
  it('writes one JSON with every conversation in it', () => {
    const all = JSON.parse(readFileSync(join(EXPORT_DIR, `${EXPORT_PREFIX}.json`), 'utf-8'));
  ...
  it('zips the pairs, with a README on top', async () => {
    const path = join(EXPORT_DIR, `${EXPORT_PREFIX}-export.zip`);
    expect(statSync(path).size).toBeGreaterThan(100_000);
    const zip = await JSZip.loadAsync(readFileSync(path));
    const names = Object.keys(zip.files);
    expect(names).toContain('README.md');
    for (const conv of conversations) {
      expect(names, conv.id).toContain(`${EXPORT_PREFIX}-${conv.id}.md`);
      expect(names, conv.id).toContain(`${EXPORT_PREFIX}-${conv.id}.json`);
    }
  });
});
