...
const SITE = 'https://turmadomaster.vercel.app';
...
describe('one page per conversation', () => {
  ...
  it('is titled and canonical for that conversation, not the home page', () => {
    for (const conv of conversations) {
      ...
      expect(html, conv.id).toMatch(new RegExp(`<title>Daniel Vorcaro ↔ .+ — ${BRAND_NAME}<\\/title>`));
    }
  });
  ...
  it('names the report by pages and hash', () => {
    ...
    expect(html).toContain('href="https://github.com/LuisHNovais/turma-do-master/blob/main/data/source/');
  ...
  it('shows only a preview of the big one and says where the rest is', () => {
    ...
    expect(html).toContain(`/export/${EXPORT_PREFIX}-martha-graeff.md`);
  ...
  it('describes itself as a Conversation in the dataset', () => {
    for (const conv of conversations) {
      ...
      expect(ld.encoding.map(e => e.contentUrl)).toContain(`${SITE}/export/${EXPORT_PREFIX}-${conv.id}.md`);
...
describe('a conversation too big for one page', () => {
  ...
  it('holds only that month, in order, chained to its neighbours', () => {
    ...
    expect(html).toContain('<link rel="canonical" href="https://turmadomaster.vercel.app/chat/martha-graeff/2024-12">');
    expect(html).toContain('<link rel="prev" href="https://turmadomaster.vercel.app/chat/martha-graeff/2024-11">');
    expect(html).toContain('<link rel="next" href="https://turmadomaster.vercel.app/chat/martha-graeff/2025-01">');
...
describe('people', () => {
  ...
  it('lists every mention by conversation, dated, pointing at the message', () => {
    ...
    expect(html).toContain('href="https://turmadomaster.vercel.app/#/chat/ciro-soares/msg/34"');
...
describe('the home page', () => {
  it('describes the corpus as a Dataset with downloads', () => {
    ...
    expect(dataset.distribution.map(d => d.contentUrl)).toContain(`${SITE}/export/${EXPORT_PREFIX}-export.zip`);
  });
});
...
describe('llms.txt', () => {
  it('quotes the real size of the big files', () => {
    ...
    const real = (statSync(join(DIST, `export/${EXPORT_PREFIX}.md`)).size / 1e6).toFixed(1);
    expect(t).toContain(`${EXPORT_PREFIX}.md (${real} MB)`);
  });
});
...
describe('llms-full.txt', () => {
  ...
  it('names every conversation and points at its Markdown', () => {
    ...
    for (const conv of conversations) {
      ...
      expect(t, conv.id).toContain(`${SITE}/export/${EXPORT_PREFIX}-${conv.id}.md`);
    }
  });
...
describe('sitemap.xml', () => {
  it('lists home, every page and every Markdown', () => {
    ...
    for (const conv of conversations) {
      ...
      expect(locs, conv.id).toContain(`${SITE}/export/${EXPORT_PREFIX}-${conv.id}.md`);
    }
