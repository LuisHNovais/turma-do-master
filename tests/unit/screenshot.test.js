...
describe('screenshotFilename()', () => {
  it('builds a name from the contact', () => {
    expect(screenshotFilename('Ciro Soares')).toBe(`${EXPORT_PREFIX}-ciro-soares.png`);
  });

  it('strips accents and punctuation', () => {
    expect(screenshotFilename('Alexandre de Moraes BRASILIA'))
      .toBe(`${EXPORT_PREFIX}-alexandre-de-moraes-brasilia.png`);
    expect(screenshotFilename('Luiz Rennó')).toBe(`${EXPORT_PREFIX}-luiz-renno.png`);
    expect(screenshotFilename('DV (autoenvio)')).toBe(`${EXPORT_PREFIX}-dv-autoenvio.png`);
  });

  it('falls back when there is no usable name', () => {
    expect(screenshotFilename('')).toBe(`${EXPORT_PREFIX}-conversa.png`);
    expect(screenshotFilename('!!!')).toBe(`${EXPORT_PREFIX}-conversa.png`);
    expect(screenshotFilename(undefined)).toBe(`${EXPORT_PREFIX}-conversa.png`);
  });
});
...
describe('delivery', () => {
  ...
  it('returns the image for preview when nothing can take it', async () => {
    ...
    expect(result.filename).toBe(`${EXPORT_PREFIX}-ciro.png`);
    expect(clickSpy).not.toHaveBeenCalled();
  });
