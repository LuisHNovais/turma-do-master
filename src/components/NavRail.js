...
const ICON_GAME = `<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M7 11h10a5 5 0 0 1 4.7 3.3l.6 1.8a2.8 2.8 0 0 1-4.7 2.8L15.3 16H8.7l-2.3 2.9a2.8 2.8 0 0 1-4.7-2.8l.6-1.8A5 5 0 0 1 7 11Z"/><path d="M8 9V6"/><path d="M16 9V6"/><path d="M8 14h.01"/><path d="M11 14h.01"/><path d="M16 14h.01"/></svg>`;
 ...
 * @returns {HTMLElement}
 */
export function renderNavRail(container, { avatarSrc, onSettings, onChat, onCalls, onGame } = {}) {
  ...
  const navItems = [
    { icon: ICON_CHAT, label: 'Conversas', active: true, enabled: true, chats: true },
    ...
    { icon: ICON_GAME, label: 'Jogo', active: false, enabled: true, game: true },
  ];
  ...
  for (const item of navItems) {
    ...
    if (item.chats && onChat) btn.addEventListener('click', onChat);
    if (item.calls && onCalls) btn.addEventListener('click', onCalls);
    if (item.game && onGame) btn.addEventListener('click', onGame);
  ...
  rail.setActive = (which) => {
    for (const btn of rail.querySelectorAll('button[aria-label]')) {
      ...
      if (label === 'Jogo') btn.classList.toggle('active', which === 'game');
    }
  };
