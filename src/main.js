// A Turma do Master — leaked-message viewer
...
async function init() {
  ...
  // scripts/split-data.mjs); this is only the fallback for older data.
  ...
  function showGame() {
    ...
    container.classList.add('chat-open');
    while (mainArea.firstChild) mainArea.removeChild(mainArea.firstChild);
    activeGamePanel = renderGamePanel(mainArea, {
      onBack: () => router.navigate('home'),
    });
  }
  ...
  const navRail = renderNavRail(container, {
    ...
    onGame: () => router.navigate('game'),
    avatarSrc: '/assets/avatar-dv.jpg',
    onSettings: openSettings,
    onChat: () => {
      // From the calls screen, this is the way back to the list.
      if (['calls', 'game'].includes(router.getCurrentRoute().route)) { router.navigate('home'); return; }
      closeProfile();
      closeSettings();
    },
  });
  ...
  router.on('game', () => { sidebar.showGame?.(); navRail.setActive?.('game'); showGame(); });
...
init().catch(err => {
  console.error('A Turma do Master init failed:', err);
